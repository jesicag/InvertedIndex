...
fileKJV = ('invertedIndex/mainFunction/bible_xml/KJV.xml')
bibleKJV = readBible(fileKJV)
bookNameKJV = bibleBookName(fileKJV)
noVersKJV = bibleNoVers(fileKJV)
versesKJV = bibleVers(fileKJV)
tokenKJV = tokenization(versesKJV)
caseFoldKJV = caseFolding(tokenKJV)
stopwordDelKJV = stopwordRemove(caseFoldKJV)
stemKJV = stemming(stopwordDelKJV)
uniqTermsKJV = uniqueWords(stemKJV)
indexKJV = createIndex(stopwordDelKJV, noVersKJV)
# =============================================================================
# PROCESS FOR DOCUMENTS
# =============================================================================
def mainKJV(textKJV):
    l = queryInIndex(processQuery(textKJV),indexKJV)
    print(l)
    N = len(noVersKJV)
    docFrequency = df(l, indexKJV)
    invDocFrequency = idf(docFrequency, N)
    termFrequency = tf(l, indexKJV)
    TFIDF = tfidf(termFrequency, invDocFrequency)
    sc = score(TFIDF)
    result = []
    for i in range(len(sc)):
        docLoc = noVersKJV.index(sc[i])
        result.append((noVersKJV[docLoc],versesKJV[docLoc]))
    return result