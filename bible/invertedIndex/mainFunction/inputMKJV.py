...
fileMKJV = ('invertedIndex/mainFunction/bible_xml/MKJV.xml')
bibleMKJV = readBible(fileMKJV)
bookNameMKJV = bibleBookName(fileMKJV)
noVersMKJV = bibleNoVers(fileMKJV)
versesMKJV = bibleVers(fileMKJV)
tokenMKJV = tokenization(versesMKJV)
caseFoldMKJV = caseFolding(tokenMKJV)
stopwordDelMKJV = stopwordRemove(caseFoldMKJV)
stemMKJV = stemming(stopwordDelMKJV)
uniqTermsMKJV = uniqueWords(stemMKJV)
indexMKJV = createIndex(stopwordDelMKJV, noVersMKJV)
# =============================================================================
# PROCESS FOR DOCUMENTS
# =============================================================================
def mainMKJV(textMKJV):
    l = queryInIndex(processQuery(textMKJV),indexMKJV)
    print(l)
    N = len(noVersMKJV)
    docFrequency = df(l, indexMKJV)
    invDocFrequency = idf(docFrequency, N)
    termFrequency = tf(l, indexMKJV)
    TFIDF = tfidf(termFrequency, invDocFrequency)
    sc = score(TFIDF)
    result = []
    for i in range(len(sc)):
        docLoc = noVersMKJV.index(sc[i])
        result.append((noVersMKJV[docLoc],versesMKJV[docLoc]))
    return result