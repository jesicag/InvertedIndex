...
fileNHEB = ('invertedIndex/mainFunction/bible_xml/NHEB.xml')
bibleNHEB = readBible(fileNHEB)
bookNameNHEB = bibleBookName(fileNHEB)
noVersNHEB = bibleNoVers(fileNHEB)
versesNHEB = bibleVers(fileNHEB)
tokenNHEB = tokenization(versesNHEB)
caseFoldNHEB = caseFolding(tokenNHEB)
stopwordDelNHEB = stopwordRemove(caseFoldNHEB)
stemNHEB = stemming(stopwordDelNHEB)
uniqTermsNHEB = uniqueWords(stemNHEB)
indexNHEB = createIndex(stopwordDelNHEB, noVersNHEB)
# =============================================================================
# PROCESS FOR DOCUMENTS
# =============================================================================
def mainNHEB(textNHEB):
    l = queryInIndex(processQuery(textNHEB),indexNHEB)
    print(l)
    N = len(noVersNHEB)
    docFrequency = df(l, indexNHEB)
    invDocFrequency = idf(docFrequency, N)
    termFrequency = tf(l, indexNHEB)
    TFIDF = tfidf(termFrequency, invDocFrequency)
    sc = score(TFIDF)
    result = []
    for i in range(len(sc)):
        docLoc = noVersNHEB.index(sc[i])
        result.append((noVersNHEB[docLoc],versesNHEB[docLoc]))
    return result